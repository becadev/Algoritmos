...
//     // tabela
//     for (int i = 0; i < l; i++) { // index da linha
//         cout << setw(5) << p[i] << "|"; 
//         for (int j = 0; j <= v; j++) {
//             cout << setw(5) << dp[j];
//         }
...
int main(){
    ...
    // dp.resize(v+1, 0);
    dp.resize(v+10, std::vector<int>(v+10, -1));
    for(int i = 0 ; i < n ; i ++)
        cin >> p[i];
    for(int i = 0 ; i < n ; i ++)
        cin >> pag[i];
    ...
    auto elapsed = duration_cast<seconds>(end - beg);
    // cerr << "Tempo de execução sem Programação dinâmica em segundos: " << elapsed.count() << endl;
